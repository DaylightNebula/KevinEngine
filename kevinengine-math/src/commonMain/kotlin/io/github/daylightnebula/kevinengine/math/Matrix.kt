...
data class Mat4(
    ...
    fun toFloatArrayColumnAligned(): FloatArray = floatArrayOf(
...
fun scale(x: Float, y: Float, z: Float) = scale(Float3(x, y, z))
fun scale(f: Float) = scale(Float3(f))
fun scale(s: Float3) = Mat4(Float4(x = s.x), Float4(y = s.y), Float4(z = s.z))
fun scale(m: Mat4) = scale(m.scale)

fun translation(x: Float, y: Float, z: Float) = translation(Float3(x, y, z))
...
fun lookAt(eye: Float3, target: Float3, up: Float3 = Float3(0f, 1f, 0f)): Mat4 {
    var dir = eye - target
    dir *= fastInverseSqrt(dir.x * dir.x + dir.y * dir.y + dir.z * dir.z)
    var left = Float3(up.y * dir.z - up.z * dir.y, up.z * dir.x - up.x * dir.z, up.x * dir.y - up.y * dir.x)
    left *= fastInverseSqrt(left.x * left.x + left.y * left.y + left.z * left.z)
    val up = Float3(dir.y * left.z - dir.z * left.y, dir.z * left.x - dir.x * left.z, dir.x * left.y - dir.y * left.x)

    return Mat4(
        Float4(left.x, up.x, dir.x, 0f),
        Float4(left.y, up.y, dir.y, 0f),
        Float4(left.z, up.z, dir.z, 0f),
        Float4(
...
// doom style fast inverse square root
...
fun perspective(fov: Float, ratio: Float, near: Float, far: Float): Mat4 {
    ...
    return Mat4(
        Float4(1f / (h * ratio), 0f, 0f, 0f),
        Float4(0f, 1f / h, 0f, 0f),
        Float4(0f, 0f, (far + near) / (near - far), -1f),
        Float4(0f, 0f, (far + far) * near / (near - far), 0f)
    )
}
